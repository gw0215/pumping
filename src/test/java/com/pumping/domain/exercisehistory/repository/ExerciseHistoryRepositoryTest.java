...
class ExerciseHistoryRepositoryTest {
    ...
    void 월별_운동부위별_총_볼륨_조회() {
        ...
        int reps2 = 5;
        int sets2 = 2;
        double expectedVolumeLastMonth = weight2 * reps2 * sets2;
        ...
        List<MonthlyPartVolumeDto> volumes = exerciseHistoryRepository.findMonthlyVolumeByPart(member.getId(), lastMonthDate.minusDays(1), thisMonthDate.plusDays(1));

        Assertions.assertThat(volumes).hasSize(2);

        Map<Integer, Double> monthToVolume = volumes.stream().collect(Collectors.toMap(MonthlyPartVolumeDto::getMonth, MonthlyPartVolumeDto::getTotalVolume));

        Assertions.assertThat(monthToVolume.get(lastMonthDate.getMonthValue())).isEqualTo(expectedVolumeLastMonth);
        Assertions.assertThat(monthToVolume.get(thisMonthDate.getMonthValue())).isEqualTo(expectedVolumeThisMonth);
    }
